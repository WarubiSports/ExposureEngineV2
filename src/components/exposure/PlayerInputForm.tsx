...
import type { PlayerProfile, SeasonStat, ExposureEvent, Position, YouthLeague, AthleticRating } from '@/types';
import { LEAGUES, POSITIONS, ATHLETIC_RATINGS } from '@/types';
...
const SectionHeader = ({ step, title, icon: Icon }: { step: string, title: string, icon: React.ElementType }) => (
...
export const PlayerInputForm = ({ onSubmit, isLoading }: Props) => {
  ...
  const handleInputChange = (field: keyof PlayerProfile, value: PlayerProfile[keyof PlayerProfile]) => {
    setProfile(prev => ({ ...prev, [field]: value }));
  };

  const handleAcademicChange = (field: keyof typeof profile.academics, value: string | number) => {
    setProfile(prev => ({
      ...prev,
  ...
  const handleAthleticChange = (field: keyof typeof profile.athleticProfile, value: AthleticRating) => {
    setProfile(prev => ({
      ...prev,
  ...
  const updateSeason = (index: number, field: keyof SeasonStat, value: SeasonStat[keyof SeasonStat]) => {
  ...
  const updateEvent = (index: number, field: keyof ExposureEvent, value: string) => {
      ...
      <div className="bg-white dark:bg-slate-900/60 backdrop-blur-sm p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-white/5 shadow-lg dark:shadow-black/20">
            ...
            <Label>Adult / International Experience</Label>
             <div className="relative">
              <select
                ...
                onChange={(e) => handleInputChange('experienceLevel', e.target.value as PlayerProfile['experienceLevel'])}
              >
        ...
        <div className="border-t border-slate-200 dark:border-white/5 pt-6">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {([
                { label: 'Speed (Accel / Top Speed)', key: 'speed' as const },
                { label: 'Strength / Physicality', key: 'strength' as const },
                { label: 'Endurance / Stamina', key: 'endurance' as const },
                { label: 'Work Rate / Intensity', key: 'workRate' as const },
                { label: 'Technical Ability', key: 'technical' as const },
                { label: 'Tactical Understanding', key: 'tactical' as const }
              ]).map((attr) => (
                 <div key={attr.key}>
                    ...
                    <div className="relative">
                      <select
                        ...
                        value={profile.athleticProfile[attr.key]}
                        onChange={(e) => handleAthleticChange(attr.key, e.target.value as AthleticRating)}
                      >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                 ...
                 <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateSeason(idx, 'mainRole', e.target.value as SeasonStat['mainRole'])}
                    >
